...
impl TrayIcon {
    pub fn new(id: TrayIconId, attrs: TrayIconAttributes) -> crate::Result<Self> {
        // AppIndicator exposes this value as StatusNotifierItem.Id. Decorating
        // it with an implementation detail makes shells display names such as
        // "tray-icon tray app ursamarkup-tray" when they fall back to the ID.
        ...
        if let Some(title) = attrs.title {
            // The AppIndicator title is the human-readable application name
            // consumed by StatusNotifierItem hosts. The label is separate
            // panel text and should not be used as the application identity.
            indicator.set_title(title.as_str());
        }
    ...
    pub fn set_title<S: AsRef<str>>(&mut self, title: Option<S>) {
        self.indicator
            .set_title(title.as_ref().map(|t| t.as_ref()).unwrap_or(""));
    }
