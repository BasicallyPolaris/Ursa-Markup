...
import type { StrokeGroup, Tool, BrushSettings, Point } from '../types'

// Maximum number of stroke groups in history
const MAX_HISTORY = 100

// Replay a single stroke onto a canvas context
export const replayStroke = (ctx: CanvasRenderingContext2D, stroke: {
  tool: Tool
  points: Point[]
  brush: BrushSettings
...
export function usePerTabStrokeHistory({
  ...
  const replayStrokes = useCallback((canvas: HTMLCanvasElement, upToIndex: number) => {
  ...
  const startStroke = useCallback((tool: Tool, brush: BrushSettings, point: Point, blendMode: 'normal' | 'color' | 'multiply' = 'normal') => {
